...
dtrump = tf.data.Dataset.from_tensor_slices(trump_train).repeat().batch(batch_size)
dcage = tf.data.Dataset.from_tensor_slices(cage_train).repeat().batch(batch_size)

# create an initializable iterator
iterator_trump = dtrump.make_initializable_iterator()
iterator_cage = dcage.make_initializable_iterator()

# extract an element
next_cage = iterator_cage.get_next()
next_trump = iterator_trump.get_next()
print('next cage shape: {}, next cage dtype {}'.format(next_cage.shape,next_cage.dtype))
print('next trump shape: {}, next trump dtype {}'.format(next_trump.shape,next_trump.dtype))
...
def enc(p1,p2):
	...
	'''
    p1l1 = tf.layers.conv2d(p1,filters =8 ,kernel_size = [5,5],padding='same',activation = tf.nn.relu,kernel_initializer=tf.truncated_normal_initializer(stddev=0.1))
    p1l1 = tf.layers.average_pooling2d(p1l1,pool_size = [2,2],strides = 2)
    p1l1 = tf.layers.conv2d(p1l1,filters =16 ,kernel_size = [5,5],padding='same',activation = tf.nn.relu,kernel_initializer=tf.truncated_normal_initializer(stddev=0.1))
    p1l1 = tf.layers.average_pooling2d(p1l1,pool_size = [2,2],strides = 2)
    p1l1 = tf.reshape(p1l1,[-1,64*64*16])
    p1l1 = tf.layers.dense(p1l1,units = 1000,activation=tf.nn.relu)
    p1encoded = tf.layers.dense(p1l1,units = 2048)
    p2l1 = tf.layers.conv2d(p2,filters =8 ,kernel_size = [5,5],padding='same',activation = tf.nn.relu,kernel_initializer=tf.truncated_normal_initializer(stddev=0.1))
    p2l1 = tf.layers.average_pooling2d(p2l1,pool_size = [2,2],strides = 2)
    p2l1 = tf.layers.conv2d(p2l1,filters =16 ,kernel_size = [5,5],padding='same',activation = tf.nn.relu,kernel_initializer=tf.truncated_normal_initializer(stddev=0.1))
    p2l1 = tf.layers.average_pooling2d(p2l1,pool_size = [2,2],strides = 2)
    p2l1 = tf.reshape(p2l1,[-1,64*64*16])
    p2l1 = tf.layers.dense(p2l1,units = 1000,activation=tf.nn.relu)
    p2encoded = tf.layers.dense(p2l1,units = 2048)
...
''''''
# total variation denoising
loss_tv = lambda_tv * total_variation_regularization(p2ext)

# total loss
p2loss = loss_s + loss_f + loss_tv
...
train_cage = tf.data.Dataset.zip((dcage)).shuffle(500).repeat().batch(batch_size)
train_trump = tf.data.Dataset.zip((dtrump)).shuffle(500).repeat().batch(batch_size)
train_1 = iterator_cage.make_initializer(dcage)
train_2 = iterator_trump.make_initializer(dtrump)
...
with tf.Session(config = config) as sess:  
    ...
    print('\n')
    tr,ca = sess.run([p1ext, p2ext], feed_dict={p1: trump[300:300 + batch_size], p2: cage[300:300 + batch_size]})
    ...
    # ca = np.asarray(ca)
    print('\n')
    print('trump[301] shape: ',trump[301].shape)
    print('tr shape: ',tr.shape)
    cv2.imshow('trump',trump[301])
    cv2.imshow('trump1',tr[1])
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    cv2.imshow('cage',cage[301])
    cv2.imshow('cage1',ca[1])
    cv2.waitKey(0)
    cv2.destroyAllWindows()
